"use client";

import React from "react";
import { useRouter } from "next/navigation";
import { ArrowLeft, Calendar, Clock, MapPin, User, CreditCard, CheckCircle, MessageCircle, XCircle, Star } from 'lucide-react';
import Head from 'next/head';

type OrderStatus = "Menunggu" | "Diproses" | "Selesai";

interface OrderDetail {
  id: number;
  jenis: string;
  tipe: string;
  tanggal: string;
  waktu: string;
  petugas: string;
  alamat: string;
  catatan?: string;
  status: OrderStatus;
  harga: string;
  metodePembayaran: string;
  nomorPesanan: string;
  estimasiSelesai: string;
  namaPetugas?: string;
  fotoPetugas?: string;
  ratingPetugas?: number;
}

const OrderDetailPage: React.FC = () => {
  const router = useRouter();

  // Data dummy - nanti bisa diganti dengan data dari API/props
  const order: OrderDetail = {
    id: 2,
    jenis: "Pembersihan + Pewangi",
    tipe: "Pembersihan Deep Clean",
    tanggal: "Rabu, 12 November 2025",
    waktu: "14:00",
    petugas: "Perempuan",
    alamat: "Jl. Sudirman No. 123, Kamar 205",
    catatan: "Tolong fokus di area kamar mandi",
    status: "Diproses",
    harga: "Rp 150.000",
    metodePembayaran: "Transfer Bank",
    nomorPesanan: "ORD-20251112-002",
    estimasiSelesai: "2-3 jam",
    namaPetugas: "Siti Nurhaliza",
    fotoPetugas: "/default-avatar.png",
    ratingPetugas: 4.8,
  };

  const handleWhatsApp = () => {
    const message = encodeURIComponent(
      `Halo, saya ingin menanyakan pesanan saya dengan nomor: ${order.nomorPesanan}`
    );
    window.open(`https://wa.me/6281234567890?text=${message}`, "_blank");
  };

  const handleCancel = () => {
    if (confirm("Apakah Anda yakin ingin membatalkan pesanan ini?")) {
      alert("Pesanan berhasil dibatalkan");
      router.push("/dashboard/user");
    }
  };

  const getStatusConfig = (status: OrderStatus) => {
    switch (status) {
      case "Selesai":
        return { bg: '#d1fae5', color: '#065f46', icon: CheckCircle };
      case "Diproses":
        return { bg: '#dbeafe', color: '#1e40af', icon: Clock };
      case "Menunggu":
        return { bg: '#fef3c7', color: '#92400e', icon: Clock };
      default:
        return { bg: '#f3f4f6', color: '#374151', icon: Clock };
    }
  };

  const statusConfig = getStatusConfig(order.status);
  const StatusIcon = statusConfig.icon;

  return (
    <>
      <Head>
        <title>Detail Pesanan - Sairaklin.id</title>
        <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/css/bootstrap.min.css" rel="stylesheet" />
      </Head>
      
      <div className="container py-4">
        <div className="row justify-content-center">
          <div className="col-lg-8">
            {/* Back Button */}
            <button 
              onClick={() => router.back()}
              className="btn btn-link p-0 mb-3 text-decoration-none d-flex align-items-center"
              style={{color: '#ec4899'}}
            >
              <ArrowLeft size={20} className="me-2" />
              Kembali ke Dashboard
            </button>

            {/* Status Banner */}
            <div className="card shadow border-0 mb-3" style={{
              background: 'linear-gradient(135deg, #fce7f3 0%, #dbeafe 100%)'
            }}>
              <div className="card-body p-4">
                <div className="d-flex align-items-center">
                  <div className="rounded-circle p-3 me-3" style={{
                    backgroundColor: statusConfig.bg
                  }}>
                    <StatusIcon size={32} style={{color: statusConfig.color}} />
                  </div>
                  <div>
                    <h4 className="mb-1 fw-bold">Status: {order.status}</h4>
                    <p className="mb-0 text-muted">
                      {order.status === "Selesai" && "Pesanan Anda telah selesai dikerjakan"}
                      {order.status === "Diproses" && `Petugas sedang dalam perjalanan. Estimasi ${order.estimasiSelesai}`}
                      {order.status === "Menunggu" && "Menunggu konfirmasi dari petugas"}
                    </p>
                  </div>
                </div>
              </div>
            </div>

            {/* Informasi Pesanan */}
            <div className="card shadow border-0 mb-3">
              <div className="card-body p-4">
                <h5 className="fw-bold mb-3 pb-2 border-bottom">Informasi Pesanan</h5>
                
                <div className="row g-3">
                  <div className="col-12">
                    <div className="d-flex align-items-start">
                      <div className="rounded p-2 me-3" style={{backgroundColor: '#fce7f3'}}>
                        <CheckCircle size={20} style={{color: '#ec4899'}} />
                      </div>
                      <div className="flex-grow-1">
                        <small className="text-muted d-block">Nomor Pesanan</small>
                        <strong>{order.nomorPesanan}</strong>
                      </div>
                    </div>
                  </div>

                  <div className="col-md-6">
                    <div className="d-flex align-items-start">
                      <div className="rounded p-2 me-3" style={{backgroundColor: '#dbeafe'}}>
                        <Calendar size={20} style={{color: '#3b82f6'}} />
                      </div>
                      <div className="flex-grow-1">
                        <small className="text-muted d-block">Tanggal</small>
                        <strong>{order.tanggal}</strong>
                      </div>
                    </div>
                  </div>

                  <div className="col-md-6">
                    <div className="d-flex align-items-start">
                      <div className="rounded p-2 me-3" style={{backgroundColor: '#dbeafe'}}>
                        <Clock size={20} style={{color: '#3b82f6'}} />
                      </div>
                      <div className="flex-grow-1">
                        <small className="text-muted d-block">Waktu</small>
                        <strong>{order.waktu} WIB</strong>
                      </div>
                    </div>
                  </div>

                  <div className="col-12">
                    <div className="d-flex align-items-start">
                      <div className="rounded p-2 me-3" style={{backgroundColor: '#fce7f3'}}>
                        <MapPin size={20} style={{color: '#ec4899'}} />
                      </div>
                      <div className="flex-grow-1">
                        <small className="text-muted d-block">Alamat</small>
                        <strong>{order.alamat}</strong>
                      </div>
                    </div>
                  </div>

                  <div className="col-12">
                    <div className="card border" style={{backgroundColor: '#f9fafb'}}>
                      <div className="card-body">
                        <div className="mb-2">
                          <span className="badge mb-2" style={{backgroundColor: '#fce7f3', color: '#be185d'}}>
                            {order.tipe}
                          </span>
                        </div>
                        <h6 className="fw-bold mb-1">{order.jenis}</h6>
                        <div className="text-muted small">Pilihan Petugas: {order.petugas}</div>
                        {order.catatan && (
                          <div className="mt-2 pt-2 border-top">
                            <small className="text-muted d-block">Catatan Khusus:</small>
                            <small>{order.catatan}</small>
                          </div>
                        )}
                      </div>
                    </div>
                  </div>
                </div>
              </div>
            </div>

            {/* Informasi Petugas */}
            {order.status !== "Menunggu" && order.namaPetugas && (
              <div className="card shadow border-0 mb-3">
                <div className="card-body p-4">
                  <h5 className="fw-bold mb-3 pb-2 border-bottom">Informasi Petugas</h5>
                  
                  <div className="d-flex align-items-center mb-3">
                    <img
                      src={order.fotoPetugas || "/default-avatar.png"}
                      alt={order.namaPetugas}
                      className="rounded-circle me-3"
                      style={{width: '80px', height: '80px', objectFit: 'cover'}}
                    />
                    <div className="flex-grow-1">
                      <h6 className="fw-bold mb-1">{order.namaPetugas}</h6>
                      <div className="d-flex align-items-center mb-1">
                        <Star size={16} className="me-1" style={{color: '#f59e0b', fill: '#f59e0b'}} />
                        <span className="fw-semibold">{order.ratingPetugas}</span>
                        <span className="text-muted small ms-1">/ 5.0</span>
                      </div>
                      <span className="badge" style={{backgroundColor: '#dbeafe', color: '#1e40af'}}>
                        Petugas {order.petugas}
                      </span>
                    </div>
                  </div>
                  
                  <button 
                    onClick={handleWhatsApp}
                    className="btn w-100 d-flex align-items-center justify-content-center"
                    style={{
                      backgroundColor: '#25d366',
                      color: 'white'
                    }}
                  >
                    <MessageCircle size={20} className="me-2" />
                    Hubungi Petugas via WhatsApp
                  </button>
                </div>
              </div>
            )}


            {/* Timeline */}
            <div className="card shadow border-0 mb-3">
              <div className="card-body p-4">
                <h5 className="fw-bold mb-3 pb-2 border-bottom">Timeline Pesanan</h5>
                
                <div className="position-relative" style={{paddingLeft: '40px'}}>
                  {/* Timeline Line */}
                  <div className="position-absolute" style={{
                    left: '15px',
                    top: '20px',
                    bottom: '20px',
                    width: '2px',
                    background: 'linear-gradient(to bottom, #10b981 0%, #10b981 50%, #e5e7eb 50%, #e5e7eb 100%)'
                  }}></div>

                  {/* Timeline Item 1 */}
                  <div className="mb-4 position-relative">
                    <div className="position-absolute rounded-circle d-flex align-items-center justify-content-center" style={{
                      left: '-40px',
                      top: '0',
                      width: '32px',
                      height: '32px',
                      backgroundColor: '#10b981',
                      color: 'white'
                    }}>
                      <CheckCircle size={18} />
                    </div>
                    <strong className="d-block">Pesanan Dibuat</strong>
                    <small className="text-muted">12 Nov 2025, 10:30 WIB</small>
                  </div>

                  {/* Timeline Item 2 */}
                  <div className="mb-4 position-relative">
                    <div className="position-absolute rounded-circle d-flex align-items-center justify-content-center" style={{
                      left: '-40px',
                      top: '0',
                      width: '32px',
                      height: '32px',
                      backgroundColor: order.status === "Diproses" || order.status === "Selesai" ? '#10b981' : '#e5e7eb',
                      color: order.status === "Diproses" || order.status === "Selesai" ? 'white' : '#9ca3af'
                    }}>
                      {order.status === "Diproses" || order.status === "Selesai" ? (
                        <CheckCircle size={18} />
                      ) : (
                        <Clock size={18} />
                      )}
                    </div>
                    <strong className="d-block">Petugas Ditugaskan</strong>
                    <small className="text-muted">
                      {order.status === "Diproses" || order.status === "Selesai" ? "12 Nov 2025, 11:00 WIB" : "Menunggu..."}
                    </small>
                  </div>

                  {/* Timeline Item 3 */}
                  <div className="position-relative">
                    <div className="position-absolute rounded-circle d-flex align-items-center justify-content-center" style={{
                      left: '-40px',
                      top: '0',
                      width: '32px',
                      height: '32px',
                      backgroundColor: order.status === "Selesai" ? '#10b981' : '#e5e7eb',
                      color: order.status === "Selesai" ? 'white' : '#9ca3af'
                    }}>
                      {order.status === "Selesai" ? (
                        <CheckCircle size={18} />
                      ) : (
                        <Clock size={18} />
                      )}
                    </div>
                    <strong className="d-block">Pekerjaan Selesai</strong>
                    <small className="text-muted">
                      {order.status === "Selesai" ? "12 Nov 2025, 16:00 WIB" : "Menunggu..."}
                    </small>
                  </div>
                </div>
              </div>
            </div>

            {/* Action Buttons */}
            <div className="d-grid gap-2">
              <button 
                onClick={handleWhatsApp}
                className="btn d-flex align-items-center justify-content-center text-white"
                style={{
                  background: 'linear-gradient(135deg, #25d366 0%, #128c7e 100%)'
                }}
              >
                <MessageCircle size={20} className="me-2" />
                Chat via WhatsApp
              </button>
              
              {order.status === "Menunggu" && (
                <button 
                  onClick={handleCancel}
                  className="btn btn-outline-danger d-flex align-items-center justify-content-center"
                >
                  <XCircle size={20} className="me-2" />
                  Batalkan Pesanan
                </button>
              )}
              
              {order.status === "Selesai" && (
                <button 
                  className="btn d-flex align-items-center justify-content-center text-white"
                  style={{
                    background: 'linear-gradient(135deg, #f59e0b 0%, #d97706 100%)'
                  }}
                >
                  <Star size={20} className="me-2" />
                  Berikan Ulasan
                </button>
              )}
            </div>
          </div>
        </div>
      </div>

      <style jsx global>{`
        body {
          background: linear-gradient(135deg, #fce7f3 0%, #ffffff 50%, #dbeafe 100%);
          min-height: 100vh;
          margin: 0;
        }
      `}</style>
    </>
  );
};

export default OrderDetailPage;